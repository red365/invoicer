...
function InvoiceList(props) {
  ...
  const generateListRows = (organiseByYear, pageOfInvoicesToDisplay, selectedYear, invoices) => {
    if (organiseByYear) {
      const filteredInvoices = invoices.filter(invoice => invoice.year == selectedYear);
      return _.sortBy(filteredInvoices, [function (invoice) { return parseInt(invoice.month); }]).map((invoice, i) => getRowMarkup(invoice, i));
    } else {
